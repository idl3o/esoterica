...
1. **Append to the Remainder Log** (`surface/remainder-log.md`) — one short, dated, append-only entry: what this session actually *added* — to the work, and to the other participant. Be specific; "we talked about X" is not an increment, "we found Y, which changes Z" is. Never edit prior entries.
