...
def _find_repo_root(start: Path) -> Path:
    ...
    so this survives the server living at ./mcp or ./apparatus/mcp."""
    for d in [start, *start.parents]:
        if (d / "corpus").is_dir() and (d / "constellation").is_dir():
            return d
...
def _build_state(mtime: float, build_tfidf: bool = True) -> ConstellationState:
    ...
    if build_tfidf:
        state.tfidf = _build_tfidf(CORPUS_ROOT, SEARCH_DIRS)
    return state
...
def resource_zeitgeist_latest() -> str:
    zdir = CORPUS_ROOT / "synthesis" / "zeitgeist"
    if not zdir.exists():
