...
>
> **REFLECT PASS, WAVE 1 — 5 September 2026.** The first ten return passages, hand-written into the older documents the harvest cites most and which never cited back: the-between (carrying the priced 17 March convergence in its own voice), the manual of ascendance, the-remainder, the-terrestrial-alien, only-a-coincidence, the-windowless-boundary, foam-beneath-the-form, the darshan protocol, the-cosmic-gorilla (naming its own slide-whistle lapse), states-of-consciousness-architecture (the convergence priced). Each a closing section in the document's register; sixty-nine lines added, none altered. Back-edges from the older corpus into the harvest 246 → 285; singletons unchanged at 530, as expected of a wave that adds no orphan edges; the whole-corpus Cheeger constant remains exactly zero. **Ruling (Sam):** only-a-coincidence carries `do-not-correct: true`; the return passage stands by override, recorded in its frontmatter, because it is flat and the voice keeps its position. Before this wave: the relinker repaired 22 dead relative links (six truly absent remain for a ruling), the rulings moved to `apparatus/scripts/link-rules.json` shared by harness and site, and the vault was consolidated into the esoterica2 memory key. Tooling: `apparatus/scripts/reflect-worklist.py` with its baseline and nine wave worklists under `apparatus/scripts/reflect/`. Next: the remaining twenty return passages under the same gate, then Wave 2 (synthesis-type singletons). Session paused here; the site ships with this state.
