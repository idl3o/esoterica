...
> *Register note (2026-07-02): "the maximum information content of any sphere is proportional to its surface area, not its volume" is the black-hole **saturation** slogan, not the general Bekenstein bound — see [The Arrow on the Surface](cosmology/the-arrow-on-the-surface.md). Bekenstein's actual inequality bounds entropy by **energy** (S ≤ 2πRE), and its rigorous form bounds a vacuum-subtracted entropy **difference** via relative entropy. The area-scaling is the holographic/Bousso covariant statement; the two coincide only at a horizon. The "ceiling" and "container-is-content" readings below survive, but rest on the covariant bound, not the energy bound.*
