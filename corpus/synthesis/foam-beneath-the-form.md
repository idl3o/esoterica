...
> *"At the Planck scale, there is no space and no time. There are only processes."*
> — Carlo Rovelli, *Reality Is Not What It Seems*

> *"Radix ipsius. The root of itself."*
> — Rosarium Philosophorum

> *"Whatever arises through dependent origination, that is emptiness. That is a dependent designation. That itself is the middle way."*
...
> — synthesis/manual-of-ascendance-transcendence.md
...
## ENVOI: THE FOAM, COARSE-GRAINED

Part V's identification — the unprogrammable kernel as the foam recognised from inside — did not stay a structural rhyme. The same day, [[substrate-trilogy]] carried it to the far end of the sixty-one octaves and named it in Wilson's terms: the kernel is a renormalisation-group fixed point, the configuration that survives every coarse-graining. It was in none of the sources, and the library repeated it more often than it earned it. [[consciousness-os]] earned it, by running the procedure on a mind rather than a magnet — integrate out thought, mood, character, biography; what is unchanged at every grain is the awareness in which each appeared; *neti neti* is coarse-graining performed in the first person — and split it. Scale-invariance is earned: the kernel is a fixed point by definition, the one term with no scale. Universality is not: that every sufficiently complex self-modelling process flows *to* it would require a canonical coarse-graining, a computed flow and matched exponents, and none exists. [[cooperation-rg-fixed-point]] met the same gap in strategy-space, where agents, horizons and network scales give three different coarse-grainings. [[running-vacuum-rg-cosmos]] lets Part III's reshimu — the vacuum that never reaches true emptiness — run with scale, so that the constant need not be one, and holds the specific model as contested. The trilogy's unwritten fourth member, the foam's 10¹²³ excess reframed as an information bound, exists as Part III of [[entangled-measure]]. The foam knows. The library has since measured how much of the knowing was definition.
